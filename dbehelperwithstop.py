...
class DBHelper:
  def __init__(self, dbname="todo3.sqlite"):
        self.dbname = dbname
        self.conn = sqlite3.connect(dbname)

  def setup(self):
    tblstmt = "CREATE TABLE IF NOT EXISTS cowin (owner integer PRIMARY KEY,state text, district text, stop integer DEFAULT 1)"
    ...
    stopidx = "CREATE INDEX IF NOT EXISTS stopIndex ON cowin (stop ASC)"
    self.conn.execute(tblstmt)
    self.conn.execute(stateidx)
    self.conn.execute(districtidx)
    self.conn.execute(ownidx)
    self.conn.execute(stopidx)
    self.conn.commit()
  ...
  def stop(self,owner):
    stmt = "UPDATE cowin SET stop = 0 WHERE owner = (?)"
    args = (owner,)
    self.conn.execute(stmt, args)
    self.conn.commit()

  def get_stop(self,owner):
    stmt = "SELECT stop FROM cowin WHERE owner = (?)"
